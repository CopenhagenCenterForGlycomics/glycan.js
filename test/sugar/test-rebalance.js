...
QUnit.test( 'Grafting a sugar to cause imbalance' , function( assert ) {
  var sugar = new IupacSugar();
  sugar.sequence = 'Gal(b1-3)[Gal(b1-3)][NeuAc(b1-3)]GlcNAc';

  var child = new IupacSugar();
  child.sequence = 'Gal(b1-3)GlcNAc';
  let target = sugar.leaves()[2];
  target.graft(child.root.children[0]);
  target.balance();
  assert.deepEqual(sugar.sequence,'Gal(b1-3)[Gal(b1-3)][Gal(b1-3)NeuAc(b1-3)]GlcNAc','Rebalances sugar');
});

QUnit.test( 'Grafting a sugar to cause imbalance' , function( assert ) {
  var sugar = new IupacSugar();
  sugar.sequence = 'NeuAc(b1-3)Gal(b1-3)[NeuAc(b1-3)][NeuAc(b1-3)]GlcNAc';
  ...
  target.graft(child.root.children[0]);
  target.balance();
  assert.deepEqual(sugar.sequence,'NeuAc(b1-3)Gal(b1-3)[Gal(b1-3)NeuAc(b1-3)][NeuAc(b1-3)]GlcNAc','Rebalances sugar');
});
...
QUnit.test( 'Grafting a sugar to cause imbalance' , function( assert ) {
  ...
  var child = new IupacSugar();
  child.sequence = 'NeuAc(b1-2)GlcNAc';
  let target = sugar.leaves()[1];
  target.graft(child.root.children[0]);
  target.balance();
  assert.deepEqual(sugar.sequence,'NeuAc(b1-3)Gal(b1-3)[NeuAc(b1-2)NeuAc(b1-3)NeuAc(b1-3)]GlcNAc','Rebalances sugar');
});