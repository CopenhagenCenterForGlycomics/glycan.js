...
QUnit.test( "Adding additional child to a StrictMonosaccharide on the same linkage" , function( assert ) {
	var sequence = "GlcNAc";
	var foo = new StrictMonosaccharide(sequence);
	var bar = new Monosaccharide(sequence);
	var baz = new Monosaccharide(sequence);
	foo.addChild(3,bar);
	assert.ok(foo.childAt(3) === bar,"First child is set");
	foo.addChild(3,baz);
	assert.ok(foo.childAt(3) === baz,"Second child is set");
	var kids_linkages = foo.child_linkages.get(3);
	assert.ok(bar.parent === null, "First child has parent set");
	assert.ok(baz.parent === foo, "Second child has parent set");
	assert.ok(kids_linkages[0] === baz, "Second child returned in list of linkages");
	assert.ok(kids_linkages.length == 1, "First child not returned in list of linkages");
});